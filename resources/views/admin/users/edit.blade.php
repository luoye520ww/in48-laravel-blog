...
@section('title', 'Edit User - Backend')
...
<section class="flex flex-1 flex-col gap-4">
  ...
  <form method="POST" action="{{ route('admin.users.update', [$user]) }}" class="flex flex-col gap-4 rounded border bg-gray-100 p-5">
    ...
    <div>
      ...
      <input id="name" name="name" value="{{ old('name', $user->name) }}" class="mt-1 w-full rounded border px-3 py-2">
      @error('name')
    ...
    <div>
      ...
      <input id="email" name="email" value="{{ old('email', $user->email) }}" class="mt-1 w-full rounded border px-3 py-2">
      @error('email')
      <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
      @enderror
    </div>
